...
var test = import_test.test.extend({
  testControl: async ({ page, context, browser }, use, testInfo) => {
    ...
    const getTestControl = async () => {
      if (!tcBrowser) {
        tcBrowser = await import_playwright.chromium.launch({
          headless: false
        });
    ...
    manualStep.soft = async (stepName, params = {}) => await test.step(
      ...
      async () => {
        try {
          ...
          await import_test.expect.soft(false, `Soft fail in manual step: ${err.message}`).toBeTruthy();
